function Protect-ZeroEmailDomain {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        Protect-ZeroEmailDomain -Provider Cloudflare -CloudflareApiToken $Token -WhatIf
    ...
    begin {
        ...
        try {
            foreach ($zone in (Get-ZedDnsZone -Context $providerContext -ZoneName $ZoneName)) {
                Write-Verbose "Evaluating zone '$($zone.Name)' with provider '$Provider'."

                if (Test-ZedDnsZoneHasMxRecord -Context $providerContext -Zone $zone) {
                ...
                foreach ($record in $desiredRecords) {
                    ...
                    $result = Invoke-ZedDnsTxtRecordSync -Context $providerContext -Zone $zone -Record $record
