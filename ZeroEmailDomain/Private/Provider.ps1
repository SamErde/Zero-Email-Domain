...
function Confirm-ZedDnsProviderImplemented {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Provider
...
function Get-ZedDnsProviderContext {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Provider,
    ...
    switch ($Provider.Name) {
        'Cloudflare' {
            ...
            return Get-ZedCloudflareContext -ApiToken $CloudflareApiToken -BaseUri $CloudflareBaseUri -PageSize $CloudflarePageSize
        }
        default {
            Confirm-ZedDnsProviderImplemented -Provider $Provider
        }
    }
}

function Get-ZedDnsZone {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    switch ($Context.Provider) {
        'Cloudflare' {
            Get-ZedCloudflareZone -Context $Context -ZoneName $ZoneName
        }
        default {
...
function Test-ZedDnsZoneHasMxRecord {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    switch ($Context.Provider) {
        'Cloudflare' {
            Test-ZedCloudflareZoneHasMxRecord -Context $Context -Zone $Zone
        }
        default {
            throw [System.NotSupportedException]::new("Provider '$($Context.Provider)' is not implemented.")
        }
...
function Invoke-ZedDnsTxtRecordSync {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    switch ($Context.Provider) {
        'Cloudflare' {
            Invoke-ZedCloudflareTxtRecordSync -Context $Context -Zone $Zone -Record $Record
        }
        default {
            throw [System.NotSupportedException]::new("Provider '$($Context.Provider)' is not implemented.")
        }
