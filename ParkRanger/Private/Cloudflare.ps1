function Get-ParkRangerCloudflareContext {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
...
function Join-ParkRangerQueryString {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [Parameter()]
...
function Get-ParkRangerCloudflareErrorMessage {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [Parameter(Mandatory)]
...
function Invoke-ParkRangerCloudflareRequest {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    $uri = '{0}/{1}{2}' -f $Context.BaseUri, $relativePath, (Join-ParkRangerQueryString -Query $Query)
    $requestParameters = @{
        Uri            = $uri
        Method         = $Method
    ...
    if ($response.success -ne $true) {
        throw [InvalidOperationException]::new((Get-ParkRangerCloudflareErrorMessage -Response $response))
    }

    $response
}

function Get-ParkRangerCloudflarePagedResult {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    do {
        ...
        $response = Invoke-ParkRangerCloudflareRequest -Context $Context -Method Get -Path $Path -Query $pageQuery
...
function Get-ParkRangerCloudflareZone {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    if ($ZoneName) {
        foreach ($name in $ZoneName) {
            Get-ParkRangerCloudflarePagedResult -Context $Context -Path 'zones' -Query @{ name = $name } |
                ForEach-Object {
                    [PSCustomObject]@{
                        Provider     = 'Cloudflare'
                        Id           = $_.id
    ...
    Get-ParkRangerCloudflarePagedResult -Context $Context -Path 'zones' |
        ForEach-Object {
            [PSCustomObject]@{
                Provider     = 'Cloudflare'
                Id           = $_.id
...
function Get-ParkRangerCloudflareDnsRecord {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    Get-ParkRangerCloudflarePagedResult -Context $Context -Path "zones/$ZoneId/dns_records" -Query $query
}
...
function Select-ParkRangerDnsTxtRecord {
    [CmdletBinding()]
...
function Invoke-ParkRangerCloudflareTxtRecordSync {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Context,
    ...
    $existingRecords = Get-ParkRangerCloudflareDnsRecord -Context $Context -ZoneId $Zone.Id -RecordType TXT -RecordName $recordName
    $match = Select-ParkRangerDnsTxtRecord -Record $existingRecords -DesiredRecord $Record
    ...
    if ($match.MatchType -eq 'Prefix') {
        $response = Invoke-ParkRangerCloudflareRequest -Context $Context -Method Put -Path "zones/$($Zone.Id)/dns_records/$($match.Record.id)" -Body $body
